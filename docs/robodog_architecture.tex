...
\paragraph{What exists.} Eight ROS 2 packages, 198 tests, a MuJoCo
...
tests. The robot stands, holds pose and walks in simulation.
...
robot is assembled and calibrated. The camera depth path is a placeholder in
...
$1 - \cos(2\pi s)$ vanishes at both ends, so the foot lifts off and touches down
with no horizontal velocity relative to the ground. Height follows
$\sin^2(\pi s)$, likewise zero-slope at both ends, with a small
\code{touchdown\_depth} subtracted late in swing so the foot seeks the ground
rather than hovering over an uneven floor. Stride length is not configured: it
follows from the commanded velocity and the stance duration, clamped to 45\,\%
of leg reach.
...
\toprule
\textbf{Gait} & \textbf{$v^{*}_x$} & \textbf{travelled} & \textbf{height} &
\textbf{peak tilt} & \textbf{Verdict} \\
 & (m/s) & (m in 6\,s) & (m) & (deg) & \\
\midrule
stand & 0.00 & $-0.003$ & 0.320 & 0.1 & solid; 5.5\,N$\cdot$m peak, no clamping \\
trot  & 0.30 & $-0.241$ & 0.320 & 3.0 & attitude held; steps in place \\
trot  & 0.50 & $-0.292$ & 0.311 & 10.4 & attitude held; steps in place \\
walk  & 0.15 & $+0.528$ & 0.256 & 17.0 & moves forward; attitude marginal \\
walk  & 0.30 & $-0.433$ & 0.119 & 125 & falls \\
bound & 0.30 & $-0.178$ & 0.240 & 14.7 & marginal \\
\bottomrule
\end{tabular}
\end{center}

\begin{caveat}{Locomotion does not yet track commanded velocity}
Standing is solid and the balance layer regulates attitude and height well
during stepping --- a 0.3\,m/s trot holds 3$^\circ$ of tilt at the commanded
\RdStandHeight\,mm, against 25$^\circ$ without it. But the robot steps in place
rather than travelling: trot drifts backward at about 0.04\,m/s, and only the
slow walk makes forward progress, at the cost of attitude.

The force-to-motion path is verified correct in isolation --- commanding
$+30$\,N of ground reaction with all four feet planted moves the robot forward
0.52\,m, and $-30$\,N moves it backward --- so this is a tuning and timing
problem, not a sign or architecture error.
\end{caveat}

\subsection{Thermal load, and why the gait is not yet safe for hardware}
...
10\,N$\cdot$m sustained is not. Measured over six seconds of each gait, with
the start-up transient discarded:
...
walk 0.15     & 17.0 & 15.0 &  8.5 & 141\,\% & 139 & over limit \\
trot 0.30     & 17.0 & 17.0 & 10.4 & 173\,\% & 198 & over limit \\
trot 0.50     & 17.0 & 17.0 & 12.0 & 201\,\% & 260 & over limit \\
bound 0.30    & 17.0 & 17.0 & 11.2 & 187\,\% & 228 & over limit \\
\bottomrule
\end{tabular}
\end{center}

\begin{caveat}{The current gait would destroy the actuators}
Every travelling gait sits above the \RdTorqueCont\,N$\cdot$m continuous
rating in RMS, saturates at the \RdTorquePeak\,N$\cdot$m peak for a quarter to
a third of the time, and implies steady-state winding temperatures from
139 to 260\,$^\circ$C. Neodymium magnets begin to demagnetise well below the
upper end of that range.

The diagnosis is \emph{not} that the actuators are undersized. Standing costs
3.9\,N$\cdot$m RMS --- 65\,\% of continuous, settling at a comfortable
45\,$^\circ$C --- and walking a 10\,kg robot should not cost three times the
RMS of holding it still. The excess is the controller fighting itself: the same
defect that stops the robot travelling is dissipating the torque as heat
instead of as forward motion.

The safety layer does catch it. The $I^2t$ integrator exhausts its budget in
about two seconds and derates the limit to continuous, which is exactly what it
is for --- but a robot that derates mid-stride falls over. This is the
strongest argument for the bring-up order in Section~\ref{sec:migration}:
locomotion must be fixed, against identified dynamics, before a gait is ever
commanded on hardware.

Tracked as an expected-failure test (\code{test\_trot\_is\_thermally\_sustainable})
so it turns green on its own when the gait is fixed, and measurable at any time
with \code{tools/torque\_report.py}.
\end{caveat}
...
\code{robodog\_control}     & 59 & IK exactness, Jacobian, force signs, safety, balance \\
\code{robodog\_sim}         & 54 & MJCF vs URDF consistency, world geometry, physics, thermal \\
\code{robodog\_perception}  & 17 & intrinsics, deprojection, self-occlusion, noise model \\
\midrule
\textbf{Total}              & \textbf{198} & 197 pass, 1 expected failure \\
\bottomrule
\end{tabular}
\end{center}
...
cannot drift. Five tests exist specifically because a bug got past a weaker
...
legs beside the body while every physics test passed, and the gait's thermal
load. The last is an expected failure, so the gap is tracked in the suite
rather than only in prose and turns green on its own when the gait is fixed.
